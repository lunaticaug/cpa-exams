"""
기능: 수동 구조 채우기 - 템플릿을 기반으로 내용을 수동으로 매칭
입력: 구조 템플릿과 원본 텍스트
출력: 구조화된 문서
"""
...
class ManualStructureFiller:
    ...
    def extract_problem_1_manual(self) -> str:
        """문제 1을 수동으로 정확히 추출"""
        output = []
        
        # 문제 1 제목과 소개 텍스트
        output.append("## 【문제 1】 (24점)")
...
""")
        ...
        # 물음 1
        output.append("#### (물음 1)")
        output.append("상호배분법을 적용하여 보조부문의 원가를 배분할 때, 각 제조부문에 배분되는 금액을 계산하시오.")
        output.append("""
**[답안양식]**
| 부문 | 금액 |
| --- | --- |
...
""")
        ...
        # 물음 2
        output.append("#### (물음 2)")
        output.append("㈜한국은 두 개의 보조부문(수선부문, 식당부문)을 폐쇄하고 외주업체를 통해 해당 용역을 공급받을지 여부를 검토하고 있다. 해당 의사결정에서 고려해야 하는 질적 판단기준을 3가지 서술하시오. (3줄 이내로 답하시오.)")
        
        # 물음 3
        output.append("\n#### (물음 3)")
        output.append("보조부문 원가를 이중배분율(dual-rate)을 적용하여 제조부문에 배분할 때 발생할 수 있는 문제점을 2가지 서술하시오. (2줄 이내로 답하시오.)")
...
""")
        ...
        # 물음 4-6
        output.append("#### (물음 4)")
        output.append("20x1년의 각 제조부문별 제조간접원가 배부율을 계산하시오.")
        output.append("""
**[답안양식]**
| 부문 | 배부율 |
| --- | --- |
...
""")
        ...
        output.append("#### (물음 5)")
        output.append("20x1년 말의 재공품, 제품, 매출원가 계정의 잔액을 각각 계산하시오.")
        output.append("""
**[답안양식]**
- 재공품: 
- 제품: 
...
""")
        ...
        output.append("#### (물음 6)")
        output.append("외부 전문가는 ㈜한국의 경영자에게 정상원가계산(평준화원가계산: normal costing)의 도입을 권유하고 있다. 본인이 외부 전문가라고 가정하고, 실제원가계산과 비교했을 때 정상원가계산 도입의 유용성 2가지를 의사결정 측면에서 설명하시오. (2줄 이내로 답하시오.)")
...
def main():
    ...
    output_path = "output/structured/2024_2차_원가회계_문제1_수동추출.md"
    
    # 디렉토리 생성
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    
    # 수동 추출
    filler = ManualStructureFiller(template_path, source_path)
    filler.load_files()
    
    # 문제 1 추출
    problem1_content = filler.extract_problem_1_manual()
    
    # 저장
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(problem1_content)
    
    print(f"✅ 수동 추출 완료: {output_path}")
    print("\n이 방법으로 전체 문제를 정확하게 구조화할 수 있습니다.")
    print("레이아웃이 복잡한 PDF의 경우, 자동화보다는 템플릿 기반 수동 매칭이 더 정확합니다.")
